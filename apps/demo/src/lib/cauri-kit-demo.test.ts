/**
 * Cauri is registered on the kit-demo surface as an OfficialAdapterFactory
 * (providerId + create) on the beta channel. The page passes
 * [...buildDemoAdapters(), cauriAdapterFactory] and channel="beta" to
 * PartyLayerKit, which is createPartyLayer({ adapters, channel }) underneath.
 * This exercises that same client wiring with a MOCK adapter and no network:
 *
 *  - APPEARS: with the factory registered, listWallets on beta surfaces cauri
 *    with its registry name and icon.
 *  - ABSENT: without it, the discovery-adapter entry is gated out, so a consumer
 *    who never wired cauri is not shown a dead tile.
 ...
 *
 * The real cauri entry is read from the shipped beta registry so host resolution
 * is proven against what actually ships, not an inlined copy. Each test calls
 * client.destroy() so the SDK's telemetry flush timer does not keep the process
 * alive.
 */
...
const betaPath = fileURLToPath(new URL('../../../../registry/v1/beta/registry.json', import.meta.url));
const CAURI = JSON.parse(readFileSync(betaPath, 'utf8')).wallets.find((w: { id: string }) => w.id === 'cauri');
assert.ok(CAURI, 'cauri must exist in the beta registry');
...
/** A client wired like the kit-demo (beta channel), but served the real cauri
 * entry from memory so nothing touches the network. */
function makeClient(adapters: unknown[]) {
  ...
  client.registryClient.getWallets = async () => [registryEntryToWalletInfo(CAURI, 'beta')];
...
test('APPEARS: a registered cauri factory surfaces cauri on beta with its registry name and icon', async () => {
  ...
  try {
    ...
    assert.equal(cauri!.channel, 'beta');
